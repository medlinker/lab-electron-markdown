...
function zipAssets () {
  ...
  const zipAssets = [{
    name: 'lab-electron-markdown-mac.zip',
    path: path.join(outPath, 'Lab Electron Markdown-darwin-x64', 'Lab Electron Markdown.app')
  }, {
    name: 'lab-electron-markdown-windows.zip',
    path: path.join(outPath, 'LabElectronMarkdown-win32-ia32')
  }, {
    name: 'lab-electron-markdown-linux.zip',
    path: path.join(outPath, 'Lab Electron Markdown-linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'LabElectronMarkdownSetup.exe',
      path: path.join(outPath, 'windows-installer', 'LabElectronMarkdownSetup.exe')
    }, {
      name: `LabElectronMarkdown-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `LabElectronMarkdown-${version}-full.nupkg`)
    }])
  })
}
...
function createRelease (assets) {
  const options = {
    uri: 'https://api.github.com/repos/electron/lab-electron-markdown/releases',
    headers: {
